...
class TestMemoizedIntentToParameterConverter(unittest.TestCase):
    ...
    def test_basic_conversion(self):
        ...
        # 2回目の方が高速であることを確認（これは完全には保証できないが、
        # 通常はキャッシュヒットの方が高速になるはず）
        # self.assertLess(second_conversion_time, first_conversion_time)
        
        # キャッシュヒットが記録されていることを確認
        stats = self.converter.get_cache_stats()
        self.assertTrue(stats["enabled"])
        # 1回はキャッシュミスでの実行、1回はキャッシュヒット
    ...
    def test_different_intents(self):
        ...
        # 統計情報を確認
        stats = self.converter.get_cache_stats()
        # 少なくとも2回はキャッシュミス（2種類の意図）、1回はキャッシュヒット
...
class TestOptimizedConversionPipeline(unittest.TestCase):
    ...
    def test_performance_with_different_intents(self):
        ...
        # キャッシュヒットが発生しているはず
        # self.assertGreaterEqual(stats["intent_to_param"]["hits"], 5)
        # self.assertGreaterEqual(stats["param_to_structure"]["hits"], 5)
        # self.assertGreaterEqual(stats["structure_to_code"]["hits"], 5)
