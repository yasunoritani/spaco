...
class SuperColliderInterface:
    ...
    def sanitize_code(self, sc_code, node_id):
        """SuperColliderコードをサニタイズし、安全に実行できるようにします"""
        if not sc_code:
            return sc_code
            
        # 危険な操作（サーバーシャットダウンなど）をブロック
        dangerous_patterns = [
            "Server.killAll", 
            "0.exit", 
        ...
        var maxExecutionTime = 10; // 10秒の実行時間制限
    ...
    async def execute(self, sc_code: str) -> Dict[str, Any]:
        ...
        # 結果処理
        return await self._process_execution_result(result, audio_id, node_id, sc_code)

    def _validate_input(self, sc_code):
        """入力コードを検証"""
        if not sc_code or not isinstance(sc_code, str):
            raise ValueError("実行するコードが無効です")

    async def _ensure_server_connection(self):
        """サーバー接続を確認"""
        await self._verify_server_connection()

    async def _prepare_code(self, sc_code):
        """実行用にコードを準備"""
        audio_id = str(uuid.uuid4())
        node_id = self._get_next_node_id()
        prepared_code = self.sanitize_code(sc_code, node_id)
        logger.info(f"SuperColliderコードを実行中: {prepared_code[:50]}...")
        return audio_id, node_id, prepared_code

    async def _execute_code(self, prepared_code):
        """コードを実行"""
        return await self._run_in_executor(
            self.code_executor.execute, prepared_code
        )

    async def _process_execution_result(self, result, audio_id, node_id, sc_code):
        """実行結果を処理"""
        try:
            # 実行履歴に追加
    ...
    # このメソッドはclose()メソッドと同じ機能を提供しますが、
    # 互換性のために残しています
    async def shutdown(self) -> Dict[str, Any]:
        """インターフェースをシャットダウンし、すべてのリソースを解放します"""
        try:
            await self.close()
            return {
                "status": "success",
                "message": "インターフェースは正常にシャットダウンされました",
                "returncode": 0
            }
        except Exception as e:
            logger.exception("シャットダウン中にエラーが発生しました")
            return {
                "status": "error",
                "message": f"シャットダウンエラー: {str(e)}",
                "returncode": 1
            }
